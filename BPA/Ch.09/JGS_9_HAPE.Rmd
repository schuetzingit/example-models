...
testmodel <- nimbleMCMC(code = testcode1,
                        constants = testdata,
                        inits = testinits,
                        nchains = 2, 
                        niter = 40000,
...
testmodel <- nimbleMCMC(code = testcode2,
                        constants = testdata,
                        inits = testinits,
                        nchains = 2, 
                        niter = 40000,
...
for (i in 1:nind) {
   ...
   for (t in 1:(n.occasions-1)) {
    ...
    for (t in 1:2) {
      ...
      ps[1,i,t,2] <- 1-s[t,group[i]] * rfix
      ps[1,i,t,3] <- 1-s[t,group[i]] * (1-rfix)
      
      }  
     
    for (t in 3:4) {
      
      ps[1,i,t,2] <- (1-s[t,group[i]]) * r[t-2]
      ps[1,i,t,3] <- (1-s[t,group[i]]) * (1-r[t-2])
  ...
  for (g in 1:ngroup) {
    ...
    success[g] <- s[1,g] * s[2,g] * s[3,g] * s[4,g]
...
for (i in 1:nind) { 
  ...
  for (t in f[i]:(n.occasions - 1)) {
    ...
    logit(s[i,t]) <- s.mu[t] + s.re[group[i],t]
...
for (i in 1:nind) {
  ...
  success <- s.mean[1] * s.mean[2] * s.mean[3] * s.mean[4]
...
testconstants <- list(f = f,
                      ...
                      nind = dim(rch)[1],
                      ngroup = 8,
                      group = sample(1:8, 500, replace = T),
                      rfix = 0)


testinits <- function() {list(s.mu = rnorm(4, 0, sd = 1),
                              r = rbeta(2, 1, 3), 
                              p = rbeta(2, 1, 1),
                              z = z_init_jgs)}  


testmodel <- nimbleMCMC(code = testcode5,
                        data = testdata,
                        constants = testconstants,
                        inits = testinits,
                        nchains = 2, 
                        niter = 50000,
                        monitors = c("s.mean", "s.re", "p", "r", "success"),
                        summary = TRUE)
...
## NIMBLE: testcode6 random effects on survival, group effects on recovery and detection

```{r}

testcode6 <- nimbleCode({
...
for (i in 1:nind) { 
  ...
  p[i,t] <- p.m[method[i],t]
  r[i,t] <- r.m[method[i],t]
...
for (i in 1:nind) {
   ...
   for (t in 1:(n.occasions-1)) {
    ...
    for (t in 3:4) {
      ...
      ps[1,i,t,2] <- (1-s[i,t]) * r[i,t-2]
      ps[1,i,t,3] <- (1-s[i,t]) * (1-r[i,t-2])
...
testconstants <- list(f = f,
                      ...
                      group = sample(1:8, 500, replace = T),
                      nmethods = 2,
                      method = sample(1:2, 500, replace = T),
                      rfix = 0)
...
testmodel <- nimbleMCMC(code = testcode6,
                        data = testdata,
                        constants = testconstants,
                        inits = testinits,
                        nchains = 3, 
                        niter = 50000,
                        monitors = c("s.mean", "p.m", "r.m", "success"))
...
## NIMBLE: testcode7 beta on survival, group effects on recovery and detection

```{r}

testcode7 <- nimbleCode({
...
for (i in 1:nind) { 
  ...
  for (t in f[i]:(n.occasions - 1)) {
    ...
    logit(s[i,t]) <- s.alpha[t] + beta[t] * cov[i]
...
for (t in 1:(n.occasions - 1)) {
    ...
    s.alpha[t] ~ dnorm(0, sd = 1)
    beta[t] ~ dnorm(0, sd = 2)
    s.int[t] <- ilogit(s.alpha[t])
...
for (i in 1:nind) {
  ...
  success <- s.int[1] * s.int[2] * s.int[3] * s.int[4]
...
testconstants <- list(f = f,
                      n.occasions = dim(rch)[2],
                      nind = dim(rch)[1],
                      cov = rnorm(500, 0, sd = 2),
                      nmethods = 2,
                      method = sample(1:2, 500, replace = T),
                      rfix = 0)


testinits <- function() {list(s.alpha = rnorm(4, 0, sd = 1),
                              beta = rnorm(4, 0, sd = 2),
                              #r = rbeta(2, 1, 3), 
                              #p = rbeta(2, 1, 1),
                              z = z_init_jgs)}  
...
testmodel <- nimbleMCMC(code = testcode7,
                        data = testdata,
                        constants = testconstants,
                        inits = testinits,
                        nchains = 3, 
                        niter = 50000,
                        monitors = c("s.int", "beta", "p.m", "r.m", "success"))


MCMCsummary(testmodel, 
            #HPD = TRUE, 
            #hpd_prob = 0.8,
            round = 3)


MCMCtrace(object = testmodel,
          #gvals = c(0.95, 0.85, 0.75, 0.85),
          params = "success",
          ind = T,
          pdf = F)
...
y <- dt_nonstandard(seq(0,10,0.01), 1, 5, 0.25)
plot(y)
```
